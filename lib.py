...
def low_pass_pgram(s, max_freq, method='pgram'):
    """
    Return a tuple of ndarrays representing the periodogram of the data stored in s
    for frequencies below max_freq
    """

    match method:
        case 'pgram':
            frequency, power = periodogram(s)
        case 'welch':
            frequency, power = welch(s)
        case 'lombscargle':
            frequency, power = LombScargle(np.arange(s.size), s).autopower(method='cython')
        case _:
            raise ValueError("method must be one of {'pgram', 'welch', 'lombscargle'}")
            
    indicator = np.where((frequency > 0) & (frequency < max_freq))
